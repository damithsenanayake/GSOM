...
dat = PCA(50, random_state=1).fit_transform(dat)
# dat -= dat.min()
# dat /= dat.max()
...
model = GSOM(lrst=.1, sf_max=0.9, fd = .1, radius=6, min_rad = 4., sd=0.005
             , its=20, min_fract=1., fract_start=1., labels = labels)
