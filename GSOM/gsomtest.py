...
data = pd.read_csv('../zoo.data.txt', header=None).as_matrix()

legs = data[:, 13]

train = data[:, 1:]

train = np.delete(train, 12, 1)
train = train[:, :-1]

### Bitmap  indexing for the number of legs

leg_ind = np.zeros(shape=(train.shape[0], 8))

for i in range(train.shape[0]):
    # ind = np.append(np.array([1 for j in range(legs[i])]), np.array([0 for j in range(8-legs[i])]))
    # leg_ind[i] += ind
    leg_ind[legs[i]] = 1
...
t_x = np.append(train, leg_ind, axis=1).astype(float)
gsom = GSOM(sp = 0.9)
