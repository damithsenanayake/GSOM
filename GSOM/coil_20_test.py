...
X = PCA(18).fit_transform(X)[order]
t = fread[:,-1][order]
...
model = GSOM(lrst=1.,sf=0.9999999999999999999999999999999999999999, fd = .9, radius=8., min_rad =8., sd=.0008, cluster_spacing_factor=1.,momentum=0., its=1000, labels=t, neighbor_func='epanechicov', map_structure=6)#TSNE(perplexity=40)#
...
for i, u in enumerate(unique):
    ...
    plt.scatter(xi, yi, c=colors[i], label=str(imlables[int(u)-1]), alpha=0.5, edgecolors='none', s = 8)
# plt.legend()
