...
class GSOM(object):
    ...
    def train_batch(self, X):
        ...
        for i in range(its):
            ...
            self.wd = 0.0#1./(2*r **2)#self.wdst*np.exp(-lambda_wd * (1-ntime))
            ...
            for b in range(int(n_batches)):
                ...
                for k in range(batch_size):#[np.random.permutation(range(X.shape[0]))]:
                    ...
                    self.W+= (x-self.W)*theta_d*self.lr

                    ''' Curvature Enforcement '''

                    et = timeit.default_timer()-st
                    if xix%500==0:
                        print ('\riter %i : %i / %i : batch : %i :|G| = %i : n_neis :%.4f : LR: %.4f  QE: %.4f Rrad: %.2f : wdFract: %.4f : wd_coef : %.4f'%(i+1,xix, X.shape[0], b, self.W.shape[0], r, self.lr,  self.errors.sum(), (r), 0, np.mean(0) )),' time = %.2f'%(et),
