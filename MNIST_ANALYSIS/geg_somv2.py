...
class GEGSOM(object):
    ...
    def train_batch(self, X):
        try:
            ...
            self.W = np.zeros((self.grid.shape[0], X.shape[
                1]))  # np.random.RandomState(seed=5).random_sample(size=(self.grid.shape[0], X.shape[1]))#np.random.random(size=(self.grid.shape[0], X.shape[1]))
            self.W[:, :2] = self.grid
                    ...
                    for x in X[b * batch_size:(b + 1) * batch_size]:
                        ...
                        if xix % 500 == 0:
                            print (
                                ...
                                i + 1, its, xix, X.shape[0], b, self.W.shape[0], neighbors.shape[0], self.lr,
                                self.errors.sum(),
                                str(dix), decayers.shape[0] * 1. / self.W.shape[0],
                                np.mean(wd_coef))), ' time = %.2f' % (et),
